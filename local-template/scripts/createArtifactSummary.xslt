...
<!-- Replace the existing markdown-toc div with this code -->
<div id="content-wrapper" class="d-flex flex-row justify-content-between" style="gap: 16px;">
  <div id="main-content" style="width: 80%;">
    ...
    <xsl:for-each select="f:definition/f:grouping">
      <xsl:variable name="relevantResources">
        <!-- Existing resource selection code -->
  ...
  <!-- TOC Navigation sidebar matching the style from processPages.xslt -->
      ...
      <ul class="toc-list">
        <xsl:if test="$globals='Y'">
          <li class="toc-item" style="cursor: pointer;">
            <a href="#globals" class="no-underline" data-section-id="globals">
              Globals
            </a>
          </li>
        </xsl:if>
        <xsl:for-each select="f:definition/f:grouping">
          <li class="toc-item" style="cursor: pointer;">
            <a href="#{position()}" class="no-underline" data-section-id="{position()}">
