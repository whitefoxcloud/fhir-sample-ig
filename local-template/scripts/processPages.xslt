...
<xsl:template priority="10" match="/html:div">
  <div id="content-wrapper" class="d-flex flex-row justify-content-between" style="gap: 16px;">
    <div id="main-content" style="width: 80%;">
      <xsl:apply-templates select="node()"/>
    </div>
    
    <xsl:if test="count(html:h3)>1">
      <div id="content-navigator" class="sticky-toc">
        <div class="toc-container">
          <div class="d-flex justify-content-start align-items-start content-navigation-header">
            <p>On this page</p>
          </div>
          <ul class="toc-list">
            <xsl:for-each select="html:h3">
              <li class="toc-item" style="cursor: pointer;">
                ...
                <a href="#{$link}" class="no-underline" data-section-id="{$link}">
                  <xsl:value-of select="."/>
                </a>
              </li>
            </xsl:for-each>
